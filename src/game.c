...
/*   Updated: 2024/03/11 18:58:25 by galves-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_animation *new_animation(char *name, char *path, t_win win)
{
	...
	img = new_file_img(path, win);
	if (!frames)
		frames = ft_lstnew(&img);
	else
		ft_lstadd_back(&frames, ft_lstnew(&img));

	img = new_file_img("./assets/pac_closed.xpm", win);
	if (!frames)
		frames = ft_lstnew(&img);
	else
		ft_lstadd_back(&frames, ft_lstnew(&img));
	t_animation *a = (t_animation *)malloc(sizeof(t_animation));
	if (!a)
		return (NULL);
	...
	return (a);
	// return ((t_animation){.name = ft_strdup(name), .frames = frames, .delay = 8, ._tmp_delay = 0});
}
...
void	update_animation_(t_animation * a, t_ent * ent, t_img game_img) {
	...
	printf("update_animation: Animation updated\n");
}
...
void	render_entities(t_game *g)
{
	...
	while (ent)
	{
		...
		// update_animation_(ent_ptr->animation[PLAYER_IDLE], ent_ptr, g->game_img);
...
void	destroy_entity(void *ptr)
{
	...
	// destroy_animation(ent->animation[PLAYER_IDLE]);
